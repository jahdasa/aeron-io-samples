...
import io.aeron.samples.matchingengine.config.Configuration;
...
public class PriceTimePriorityStrategy implements MatchingLogic {
    ...
    private void processOrdersInList(long price, OrderList orderList,OrderEntry aggOrder){
        ...
        while (iterator.hasNext()) {
            ...
            if(quantity > 0) {
                addTrade(aggOrder.getOrderId(), price, quantity, currentOrder.getClientOrderId(), java.time.Instant.now().toEpochMilli());
    ...
    private void addTrade(long orderId, long price, long quantity, long clientOrderId, long executedTime){
        ...
        MarketData.INSTANCE.addTrade(orderId, trade.getId(),clientOrderId,price,quantity,executedTime);
